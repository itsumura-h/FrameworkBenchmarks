...
def render_page(data: dict[str, Any], source_path: Path) -> str:
    ...
    input_path_s = str(source_path).replace("\\", "/")
    input_url = (
        "https://github.com/itsumura-h/FrameworkBenchmarks/blob/master/"
        + input_path_s
    )
    input_href = html.escape(input_url, quote=True)
    input_label = html.escape(input_url)
...
<body>
  ...
  <footer>
    <p>Input: <a href="{input_href}" style="color: white;" target="_blank" rel="noopener noreferrer"><code>{input_label}</code></a></p>
